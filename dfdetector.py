...
from albumentations import (
    Compose, FancyPCA, GaussianBlur, GaussNoise, HorizontalFlip, VerticalFlip,
...
"""
    # Deepfake Detector code implemented by Christopher Otto
    # Adaptations made by Paloma Cantero
...
parser.add_argument('--frames_experiment', default=False,
                    type=bool, help='Choose whether experimenting with the number of frames.')
...
class DFDetector():
    ...
    def benchmark(cls, dataset=None, data_path=None, method="xception_celebdf", seed=24):
        ...
        if cls.dataset == 'faceforensics':
            num_frames = 20
            # setup the dataset folders
            setup_faceforensics_benchmark(cls.data_path, cls.method)
        ...
        elif cls.method == "efficientnetb7_uadfv" or  cls.method == 'efficientnetb7_faceforensics' or cls.method == 'efficientnetb7_celebdf' or cls.method == 'efficientnetb7_dftimit_hq' or cls.method == 'efficientnetb7_dftimit_lq' or cls.method == 'efficientnetb7_dfdc':
            model, img_size, normalization = prepare_method(
                method=cls.method, dataset=cls.dataset, mode='test')
    ...
    def train_method(cls, dataset=None, data_path=None, method="xception", img_save_path=None, epochs=1, batch_size=32,
        ...
        if cls.dataset == 'dfdc':
            num_frames = 5
        elif cls.dataset == 'uadfv':    
            num_frames = 15
        elif cls.dataset == 'dftimit_lq':
            num_frames = 15
        elif cls.dataset == 'dftimit_hq':
            num_frames = 15
        elif cls.dataset == 'faceforensics':
            num_frames = 15
        ...
        if cls.method == 'transformer':
            print("TRAINING FOR TRANSFORMER")
            model, average_auc, average_ap, average_acc, average_loss = train.trainTransformer(dataset=cls.dataset, data=df_faces,
                                                                                method=cls.method, img_size=img_size, normalization=normalization, augmentations=augs,
                                                                                folds=cls.folds, epochs=cls.epochs, batch_size=cls.batch_size, lr=cls.lr, fulltrain=cls.fulltrain, num_frames=num_frames
                                                                                )
        elif cls.method == '3dcnn':
            print("TRAINING FOR 3DCNN")
            model, average_auc, average_ap, average_acc, average_loss = train.train3DCNN(dataset=cls.dataset, data=df_faces,
                                                                                method=cls.method, img_size=img_size, normalization=normalization, augmentations=augs,
                                                                                folds=cls.folds, epochs=cls.epochs, batch_size=cls.batch_size, lr=cls.lr, fulltrain=cls.fulltrain, num_frames=num_frames
                                                                                )
    ...
    @classmethod
    def frames_experiment(cls, dataset=None, data_path=None, method="efficientnetb7", img_save_path=None, epochs=1, batch_size=32,
                     lr=0.001, folds=1, augmentation_strength='weak', fulltrain=False, faces_available=False, face_margin=0, seed=24):
        """Train a deepfake detection method on a dataset."""
        if img_save_path is None:
            raise ValueError(
                "Need a path to save extracted images for training.")
        cls.dataset = dataset
        print(f"Training on {cls.dataset} dataset for frames experiment.")
        cls.data_path = data_path
        cls.method = method
        cls.epochs = epochs
        cls.batch_size = batch_size
        ...
        # detect and extract faces if they are not available already
        if cls.dataset == 'uadfv':
            addon_path = '/frames_exp/'
            if not os.path.exists(img_save_path + '/frames_exp/'):
                # create directory in save path for images
                os.mkdir(img_save_path + addon_path)
                for num_frames in [5,10,15,20,25]:
                    os.mkdir(img_save_path + '/frames_exp/'+str(num_frames))
                    os.mkdir(img_save_path + '/frames_exp/'+str(num_frames)+'/real/')
                    os.mkdir(img_save_path +'/frames_exp/'+str(num_frames)+'/fake/')
            ...
            if not os.path.exists(img_save_path + '/frames_exp_hq/'):
                # create directory in save path for face crops
                os.mkdir(img_save_path + addon_path)
                for num_frames in [5,10,15,20,25]:
                    os.mkdir(img_save_path + '/frames_exp_hq/'+str(num_frames))
                    os.mkdir(img_save_path + '/frames_exp_hq/'+str(num_frames)+'/real/')
                    os.mkdir(img_save_path + '/frames_exp_hq/'+str(num_frames)+'/fake/')
            else:
                # delete create again if it already exists with old files
                shutil.rmtree(img_save_path + '/frames_exp_hq/')
                os.mkdir(img_save_path + addon_path)
                for num_frames in [5,10,15,20,25]:
                    os.mkdir(img_save_path + '/frames_exp_hq/'+str(num_frames))
                    os.mkdir(img_save_path + '/frames_exp_hq/'+str(num_frames)+'/real/')
            ...
            if not os.path.exists(img_save_path + '/frames_exp_lq/'):
                # create directory in save path for face crops
                os.mkdir(img_save_path + addon_path)
                for num_frames in [5,10,15,20,25]:
                    os.mkdir(img_save_path + '/frames_exp_lq/'+str(num_frames))
                    os.mkdir(img_save_path + '/frames_exp_lq/'+str(num_frames)+'/real/')
                    os.mkdir(img_save_path + '/frames_exp_lq/'+str(num_frames)+'/fake/')
            else:
                # delete create again if it already exists with old files
                shutil.rmtree(img_save_path + '/frames_exp_lq/')
                os.mkdir(img_save_path + addon_path)
                for num_frames in [5,10,15,20,25]:
                    os.mkdir(img_save_path + '/frames_exp_lq/'+str(num_frames))
                    os.mkdir(img_save_path + '/frames_exp_lq/'+str(num_frames)+'/real/')
        ...
        for num_frames in [5,10,15,20,25]:
            for idx, row in tqdm(df.iterrows(), total=df.shape[0]):
                ...
                if cls.dataset == 'uadfv':
                    if label == 1:
                        video = video[-14:]
                        save_dir = os.path.join(
                            img_save_path + '/frames_exp/'+str(num_frames)+'/fake/')
                    else:
                        video = video[-9:]
                        save_dir = os.path.join(
                            img_save_path + '/frames_exp/'+str(num_frames)+'/real/')
                elif cls.dataset == 'faceforensics':
                    if label == 1:
                        video = video[-11:]
                        save_dir = os.path.join(
                    ...
                    if label == 1:
                        save_dir = os.path.join(
                            img_save_path + '/frames_exp_hq/'+str(num_frames)+'/fake/')
                    else:
                        save_dir = os.path.join(
                            img_save_path + '/frames_exp_hq/'+str(num_frames)+'/real/')
                elif cls.dataset == 'dftimit_lq':
                    vid_name = row.loc['videoname']
                    video = vid_name
                    if label == 1:
                        save_dir = os.path.join(
                            img_save_path + '/frames_exp_lq/'+str(num_frames)+'/fake/')
                    else:
                        save_dir = os.path.join(
                            img_save_path + '/frames_exp_lq/'+str(num_frames)+'/real/')
            ...
            results.append([model, average_auc, average_ap, average_acc, average_loss])
...
def prepare_method(method, dataset, mode='train'):
    ...
    if method == 'xception' or method == 'xception_uadfv' or method == 'xception_celebdf' or method == 'xception_dftimit_hq' or method == 'xception_dftimit_lq' or method == 'xception_dfdc':
        ...
        if mode == 'test':
            ...
            model = None
            return model, img_size, normalization
    elif method == 'efficientnetb7' or method == 'efficientnetb7_uadfv' or method == 'efficientnetb7_faceforensics' or  method == 'efficientnetb7_celebdf' or method == 'efficientnetb7_dftimit_hq' or method == 'efficientnetb7_dftimit_lq' or method == 'efficientnetb7_dfdc':
        # 380 image size as introduced here https://www.kaggle.com/c/deepfake-detection-challenge/discussion/145721
        img_size = 380
        normalization = 'imagenet'
        if mode == 'test':
            if method == 'efficientnetb7_uadfv' or method == 'efficientnetb7_faceforensics' or method == 'efficientnetb7_celebdf' or method == 'efficientnetb7_dftimit_hq' or method == 'efficientnetb7_dftimit_lq' or method == 'efficientnetb7_dfdc':
        ...
        if mode == 'test':
            ...
            model = None
            return model, img_size, normalization
    elif method == 'transformer' or method == '3dcnn':
        img_size = 224
        normalization = 'imagenet'
        if mode == 'test':
                model_params = torch.load(
    ...
    if not test_data:
        if dataset == 'uadfv':
            ...
            if not face_crops:
                ...
                if method == 'resnet_lstm' or method == 'efficientnetb1_lstm' or method == 'transformer' or method =='3dcnn':
            ...
            if not face_crops:
                # read csv for videos
                test_dat = pd.read_csv(os.getcwd(
                ) + "/deepfake_detector/data/faceforensics_test.csv", names=['video'], header=None)
                test_list = test_dat['video'].tolist()
...
def structure_faceforensics_files(path_to_data):
    """Creates test folders and moves test videos there."""
    os.mkdir(path_to_data + '/test/')
    os.mkdir(path_to_data + '/test/fake/')
    os.mkdir(path_to_data + '/test/real/')
    test_data = pd.read_csv(
        os.getcwd() + "/deepfake_detector/data/faceforensics_test.csv", names=['video'], header=None)
    for idx, row in test_data.iterrows():
        if len(str(row.loc['video'])) > 8:
            # video is fake, therefore copy it into fake test folder
            shutil.copy(path_to_data + '/fake/' +
...
def prepare_sequence_data(dataset, df):
    ...
    if dataset == 'uadfv':
        ...
        for idx, row in tqdm(df.iterrows(), total=len(df)):
            if row.loc['label'] == 0:
                df.loc[idx, 'original'] = row.loc['video'][:3]
            elif row.loc['label'] == 1:
                df.loc[idx, 'original'] = row.loc['video'][:7]
...
def setup_faceforensics_benchmark(data_path, method):
    """
    Setup the folder structure of the UADFV Dataset.
    """
    if data_path is None:
        raise ValueError("""Please download the FaceForensics++ FaceSwap section.
                                Click on the link \"here\" and download the dataset. 
                                Extract the files and organize the folders follwing this folder structure:
                                ./faceforensics/
                                            fake/
                                            real/
                                """)
    if data_path.endswith("faceforensics"):
        print(
            f"Benchmarking \033[1m{method}\033[0m on the \033[1m FaceForensics \033[0m dataset with ...")
        # create test directories if they don't exist
        if not os.path.exists(data_path + '/test/'):
            structure_faceforensics_files(path_to_data=data_path)
        else:
            # check if path exists but files are not complete (from https://stackoverflow.com/a/2632251)
            num_files = len([f for f in os.listdir(
                data_path + '/test/') if os.path.isfile(os.path.join(data_path + '/test/real/', f))])
...
def main():
    ...
    elif args.frames_experiment:
        print(args)
        print(args.facecrops_available)
        results = DFDetector.frames_experiment(dataset=args.dataset, data_path=args.data_path, method=args.model_type, img_save_path=args.save_path, epochs=args.epochs, batch_size=args.batch_size,
                     lr=args.lr, folds=args.folds, augmentation_strength=args.augs, fulltrain=args.fulltrain,  face_margin=args.face_margin, faces_available=args.facecrops_available, seed=args.seed)
