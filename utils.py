...
def deleteJPG(path_to_dataset):
    ...
    for path, dirs, files in os.walk(path_to_dataset):
        for filename in files:
            if filename.endswith("_15.jpg") or filename.endswith("_16.jpg") or filename.endswith("_17.jpg") or filename.endswith("_18.jpg") or filename.endswith("_19.jpg"):
                # print(os.path.join(path, filename[:-4]))
...
def getCSVinfo(path_to_dataset):
    with open('dftimit_reals.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["path","videofolder","videoname"])
        for dirpath, subdirs, files in os.walk(path_to_dataset):
            for dir in subdirs:
                for x in os.listdir(os.path.join(dirpath, dir)):
                    if x.endswith('.avi'):
                        writer.writerow([os.path.join(dirpath, dir), x[:-4]])


def selectFaceForensics(path_to_dataset='/home/cnn3d/Escritorio/tfm/FaceForensics', percentage=0.3):
    n_elements = int(1000 * percentage)
    real_dir = os.path.join(path_to_dataset, 'real')
    fake_dir = os.path.join(path_to_dataset, 'fake')
    real_content = os.listdir(real_dir)
    real_content = [[i] for i in real_content]
    fake_content = os.listdir(fake_dir)
    fake_content = [[i] for i in fake_content]
    random.shuffle(real_content)
    random.shuffle(fake_content)
    test = real_content[:n_elements] + fake_content[:n_elements]
    random.shuffle(test)
    with open('E:/TFM/faceforensics_test.csv', 'w', newline='') as file:
        writer = csv.writer(file)
...
def editCSV(path):
    with open(path) as file_obj:
        with open(r'C:\Users\Paloma\Desktop\deepfakeDetection\deepfake_detector\data\dftimit_reals2.csv', 'w', newline='') as file:
            writer = csv.writer(file)        
            reader_obj = csv.reader(file_obj)
            for row in reader_obj:
                path = row[0][53:59]
                videofolder = row[0][59:]
                videoname = row[1]
                if videoname != 'videofolder':
                    writer.writerow([path, videofolder, videoname])
